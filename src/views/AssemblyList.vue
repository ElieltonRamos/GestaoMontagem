  ...
  <div>
    ...
    <div v-if="isLoading" class="bg-gray-800 rounded-lg shadow-lg p-12 border border-gray-700">
      <div class="flex items-center justify-center">
        ...
        <span class="ml-3 text-gray-400">Carregando montagens...</span>
      </div>
    </div>
...
const loadData = async () => {
  ...
  try {
    const [assembliesList, assemblersList] = await Promise.all([
      assembliesService.getAll(),
      assemblersService.getAll()
    ])
    
    assemblies.value = assembliesList
    assemblers.value = assemblersList
  } catch (error) {
