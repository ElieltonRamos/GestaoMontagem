...
export class AssemblersService {
  ...
  async create(name: string, phone: string, cpf?: string, address?: string): Promise<Assembler | null> {
    try {
  ...
  async update(id: string, name: string, phone: string, cpf?: string, address?: string): Promise<boolean> {
    try {
      await tauriClient.invoke('update_assembler', { id, name, phone, cpf, address })
      return true
    } catch (error) {
